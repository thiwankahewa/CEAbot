...
class BenchMarkerDetector(Node):
    def __init__(self):
        ...
        self.create_subscription(Image, "/gemini336/color/image_raw", self.cb_image, qos_profile_sensor_data)
        self.create_subscription(CameraInfo, "/gemini336/color/camera_info" , self.cb_camera_info, qos_profile_sensor_data)
    ...
    def publish_marker(self, visible, bench=0, error_px=0.0, orientation_deg=0.0, distance_m=None, perspective_x_deg=None, perspective_y_deg=None):
            distance_mm = -1 if distance_m is None else int(round(distance_m * 1000.0))
            perspective_x_cdeg = (-32768 if perspective_x_deg is None else int(round(perspective_x_deg * 100.0)))
            perspective_y_cdeg = (-32768 if perspective_y_deg is None else int(round(perspective_y_deg * 100.0)))
            self.publisher.publish(Int16MultiArray(data=[ 1 if visible else 0, int(bench), int(round(error_px)), int(round(orientation_deg)), distance_mm, perspective_x_cdeg, perspective_y_cdeg, ]))
    ...
    def cb_image(self, msg):
        ...
        self.publish_marker( True, bench, error_px, orientation_deg, distance_m, perspective_x_deg, perspective_y_deg)
    ...
    def manage_stream(self):
        ...
        self.stream_request_future = self.stream_client.call_async( SetBool.Request(data=self.stream_desired))
